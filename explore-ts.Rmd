...
nc_history_ts <- nc_history_detail %>%
  ...
  group_by(facility_name, site_type_simple, overnight_date) %>%
  summarize(n_overnight = n()) %>% # number of sites booked overnight
  ungroup() %>%
  as_tsibble(key = c(facility_name, site_type_simple), index = overnight_date) %>%
  fill_gaps(n_overnight = 0L)
```

```{r eval=FALSE}
# can I consolidate nc_history_dow and nc_history_detail?
# Both have 1878426 rows while nc_history_detail includes more information about each row (facility name, site_type_simple, etc.) 
...
nc_history_ts_features_means <- nc_history_ts_features %>%
  select(facility_name, site_type_simple, trend_strength, seasonal_strength_week) %>%
...
top_trends <- nc_history_ts_features %>%
  ...
  select(facility_name, site_type_simple, trend_strength) 
...
dcmp_top_trends <- inner_join(
  dcmp,
  top_trends %>% select(c(facility_name, site_type_simple)),
...
```{r}
ggplot() +
  geom_point(data = nc_history_ts_features %>% select(-site_type_simple), 
             aes(x = trend_strength, y = seasonal_strength_week),
             alpha = 0.1) +
  geom_point(data = nc_history_ts_features,
             aes(x = trend_strength, y = seasonal_strength_week, col = site_type_simple)
             ) + 
  geom_point(data = nc_history_ts_features_means,
             aes(x = trend_strength, y = seasonal_strength_week, col = site_type_simple),
             shape = 3, size = 5
             ) + 
  facet_wrap(vars(site_type_simple)) +
  theme(legend.position = "none") +
...
g <- nc_history_ts %>%
  ...
  facet_grid(facility_name ~site_type_simple, scales = "free_y") #+
  #theme(legend.position = "none")
...
#nc_history_ts_acf %>%
  ggplot() +
  geom_point(data = nc_history_ts_acf %>% select(-site_type_simple), 
             aes(x = acf1, y = season_acf1),
             alpha = 0.1) +
  geom_point(data = nc_history_ts_acf,
             aes(x = acf1, y = season_acf1, col = site_type_simple)
  ) +
...
# with acf10
  ...
  geom_point(data = nc_history_ts_acf %>% select(-site_type_simple), 
             ...
             alpha = 0.1) +
  geom_point(data = nc_history_ts_acf,
             aes(x = acf10, y = season_acf1, col = site_type_simple)
  ) +
  facet_wrap( ~ site_type_simple) +
  theme(legend.position = "none") +
  labs(title = "<add title here>")
...
```{r}
  ...
  geom_point(data = nc_history_ts_acf %>% select(-site_type_simple), 
             ...
             alpha = 0.1) +
  geom_point(data = nc_history_ts_acf,
             aes(x = acf1, y = acf10, col = site_type_simple)
  ) +
  facet_wrap( ~ site_type_simple) +
  theme(legend.position = "none") +
  labs(title = "<add title here>")
