...
First we have to check: what's in the data, and are there any important limitations? I found five:
...
3. Some campsites have become reservable during the years covered in this data set. For example, the NPS campgrounds at Cape Hatteras: Oregon Inlet and Frisco started taking reservations in 2015, and Cape Point started in 2016. Including these campgrounds unduly inflates the mulit-year growth rate.

4. As is visible in the per-campground trend data starting with [camping person-nights by park], there are a lot of outliers in the data in 2018, 2019, and 2020. Likely there are two causes: 
...
* Look at growth only in facilities + site_type_simple that existed in 2009. This would avoid adding to the growth rate due to facilities that converted camp sites to reservable during the years of interest (and thus started showing up in the data).

* As noted in [What's possible given these data sets?] the site-type categories {standard, tent, rv} overlap. I could recategorize standard sites to tent camping except in RV-only campsites, in which case I would allocate them to rv camping.

* In time series analysis, 

    - Use percent of max_occupancy on each day for each facility_name + site_type_simple. Is that more interesting/helpful than overnight counts? It would normalize for number of sites. However, the max_occupancy for some facilities + site_type_simple is quite low (for example, two or three group sites in a campground). Will STL find it difficult if there are large steps in the percent max_occupancy values?
    
    - Would a hierarchical time series model be helpful? We could use facility_id_group as the higher-level series and keep using facility_name + site_type_simple as the lower-level series.

* Look at trends by loc_mountains and/or loc_coast. Presumably the period of high occupancy will be more narrow in the mountains
...
* Use bookdown instead to create separate pages for each major section
