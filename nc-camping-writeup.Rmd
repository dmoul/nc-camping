...
So what are you waiting for? Find a campsite at [recreation.gov](https://https://www.recreation.gov) and make a reservation today.
...
## What's possible given this data set?
...
3. Some campsites have become reservable during the years covered in this data set. For example, the NPS campgrounds at Cape Hatteras: Oregon Inlet and Frisco started taking reservations in 2015, and Cape Point started in 2016. When looking at multi-year growth, including these campgrounds unduly inflates the growth rate.
...
```{r nc-site-count-ecdf, fig.width=9, fig.height=6, out.width="100%"}
nc_campsites %>%
...
```{r nc-cagr-person-night-and-revenue}
...
Since the small dip in 2013, growth in person_nights in (B) overall was `r percent(growth_over_interval, accuracy = 0.1)`.

Why did growth start in 2014 and keep going? There is nothing in the data that answers this question.

The compound annual growth rate (CAGR) was `r percent(cagr_person_nights, accuracy = 0.1)` for person_nights and `r percent(cagr_revenue, accuracy = 0.1)` for revenue. Growth varied by site_type:
...
cagr_for_table %>%
  ...
  gt() %>%
  tab_header(md("**NC camping compound annual growth rate<br>2013 - 2019**")) %>%
...
```{r normalizing-growth, fig.width=9, fig.height=6, out.width="100%"}
xx <- nc_camping_person_nights_and_revenue_site_type_camps_all_years %>%
  group_by(year, site_type) %>%
  ...
  ungroup() %>%
  group_by(year, site_type) %>%
  summarize(person_nights_normalized = 100 * person_nights_yearly_sum / pn_year_start) %>%
  ungroup() %>%
  mutate(year = year + 2008)

xx_yearly_sum <- nc_camping_person_nights_and_revenue_site_type_camps_all_years %>%
  mutate(pn_year_start = sum(person_nights[year == 1])) %>%
  group_by(year) %>%
  summarize(person_nights_yearly_sum = sum(person_nights),
            person_nights_normalized = 100 * person_nights_yearly_sum / pn_year_start) %>%
  ungroup() %>%
...
xx %>%
  ggplot() +
  geom_line(aes(year, person_nights_normalized, color = site_type), size = 0.5, alpha = 0.4) +
  geom_point(aes(year, person_nights_normalized, color = site_type, size = ), size = 2, alpha = 0.9) +
  geom_smooth(data = xx_yearly_sum,
              aes(year, person_nights_normalized)
              ) +
  scale_x_continuous(breaks = c(2010, 2015, 2020)) +
  scale_color_viridis_d(end = 0.8) +
  expand_limits(y = 0.8 * min(xx$person_nights_normalized)) +
  labs(title = "Person nights by site_type",
       subtitle = glue("{min(xx$year)}-{max(xx$year)}; {min(xx$year)} = 100"),
       x = NULL,
       caption = my_caption
  )
```
...
```{r nc-great-value, fig.width=9, fig.height=6, out.width="100%"}
