...
## What's possible given these data set?
...
4. As is visible in the per-campground trend data starting with [camping person-nights by park], there are a lot of outliers in the data in 2018, 2019, and 2020. Thus I have varied the end year as I saw fit to communicate trends most clearly. Likely there are two causes to these anomalies: 
...
## Focusing on federally-owned facilities in NC

So setting aside the NC State Park system, I focus here on federal facilities in NC, which amidst the oddities of 2020 provided `r comma(fy2020_summary$person_nights)` person-nights of camping and collected `r dollar(fy2020_summary$revenue)` in overnight camping fees. 
...
* For each `facility_id_group`, `n_sites` is a count of distinct `product_id` values. I assume that, in general, `product_id` values have not changed over time, and the same value is not used for multiple campsites at the same facility. This is not quite true, resulting in counts of camping sites in NC varying by up to 7%, depending how you count. For my purposes here this doesn't matter. Details are in Appendix 1 [There is some reuse of product_id values (unfortunately)]. 
...
## Trends: all facility+site type combos or a constant population?

As discussed in Appendix 1 [Reservable sites added during the years of interest], Some recorded growth is due to more inventory added to the recreation.gov reservation system, which inflates the growth rate. How much? In `r year_end` it was `r non_organic_growth_person_nights` of all person_nights.
...
p1 <- nc_camping_person_nights_and_revenue_site_type %>%
  ...
  geom_area(alpha = 0.9) +
  annotate("text", 
           x = year_lower_bound + 2.5, 
           y = 0.15 * y_axis_max_person_nights, 
           label = glue("Additions are mostly tent sites"),
           hjust = 0, size = 5, color = "white") +
...
```{r camp-by-dow-season,fig.width=15, fig.height=9, out.width="100%"}
...
Since I don't have a declared number of sites per facility+site_type that is reliable, I define percent occupancy as the percent of the maximum recorded occupancy for the campground + site type, where "maximum recorded" is over the years of interest. 
...
* Yearly: mean occupancy rate for all days of the year

<br>

Limitations

* I am looking at `r year_lower_bound`-2017, since the data in 2018 and 2019 are unreliable, and usage patterns earlier and in 2020 were not typical. In some plots I further subset the data in the interest of readability.
...
What if one considers only Friday and Saturday nights, the most popular nights for camping?
...
* As one would expect, there is tent camping earlier and later in the year outside the mountains.
* The patterns of tent camping everywhere and RV camping in the mountains have been remarkably consistent week-to-week
* Cabins in the mountains and RV sites elsewhere appear to be fully booked at least one day every week. Is this actually true? Seems more likely this is an analysis error.
...
nc_week_occupancy2 %>%
  filter(year >= year_lower_bound,
         year < 2018,
         site_type != "management") %>%
  group_by(year, site_type, loc_mountains, week_of_reservation) %>% #season, 
...
p1 <- max_occupancy_by_year_season %>%
  ...
  theme(legend.position = "none",
        panel.grid.major.x = element_blank()) +
  labs(title = "Days facilities reached max_occupancy",
       subtitle = glue("{year_start}-{year_stop}",
...
p1 <- max_occupancy_by_year_season_90pc %>%
  ...
  theme(legend.position = "none",
        panel.grid.major.x = element_blank()) +
  labs(title = "Days facilities reached 90% occupancy",
       subtitle = glue("{year_start}-{year_stop}",
...
```{r nc-great-value, fig.width=9, fig.height=6}
...
```{r nc-parks-site-types-for-table-appendix}
...
The following table is ordered by average yearly person_nights, which provides one measure of popularity. `n_years` is the number of years reservation.gov has managed reservations for the campground.
...
### Reservable sites added during the years of interest
...
The following campgrounds had at least one site_type reservable in `r year_lower_bound` and added the site_type in the year noted below:
...
## Days with occupancy at 100 and 90 percent
...
* In time series analysis (in explore-ts.Rmd; not included here at this time), 
...
<img src="https://i.creativecommons.org/l/by/4.0/88x31.png" alt="CC-BY" /> This work is licensed under a <a href="http://creativecommons.org/licenses/by/4.0/">Creative Commons Attribution 4.0 International License</a>

Code is available at https://github.com/dmoul/nc-camping under the [MIT license](https://github.com/dmoul/nc-camping/blob/main/LICENSE). 
