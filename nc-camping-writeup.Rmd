...
#   inner_join(.,
#              nc_parks_site_types %>% 
#                distinct(facility_id_group, facility_name, facility_id_list, site_type) %>%
#                mutate(name_first_word = str_extract(facility_name, "\\w+")),
...
nc_parks_site_types_for_table <- nc_history %>%
  ...
  group_by(facility_id_group, facility_name, facility_id_list) %>%
  #summarize(across(c(tent, cabin, group, equestrian, rv, management), sum)) %>%
...
nc_camping_person_nights_and_revenue_site_type %>%
  ...
  labs(title = "NC camping person_nights",
       subtitle = glue("At reserved sites {year_lower_bound}-{year_end}"),
       x = NULL,
       fill = "Site type")
```
...
# there is some reuse of product_id across site_type at single facility_ids, and maybe across facility_ids.
# so need to group_by(facility_id, site_type) before n_distinct(product_id)
# xx <- nc_history %>%
#   filter(year(start_date) >= year_lower_bound) %>%
...
orgs_for_table <- nc_history %>%
  ...
  inner_join(.,
             nc_parks_site_types %>% 
               ...
               ungroup(),
             by = c("name_first_word", "site_type")) %>% #, "site_type"
  group_by(facility_id_group, facility_name, site_type) %>% #site_type, 
...
consistent <- bind_rows(campgrounds_all_years,
                        campgrounds_not_all_years) %>%
  arrange(facility_name, site_type) 
...
xx <- consistent %>%
  ...
  ungroup()
# TODO: above is not giving me what I want
...
The following campgrounds did not offer any reservable sites in `r year_lower_bound` :

```{r}
x1 %>% select(facility_id, facility_name, first_year = year) %>%
  gt()
```



The following campgrounds had at least one site_type reserveable in `r year_lower_bound` and added the site_type noted below in a later year:

```{r}
x2 %>% select(facility_id, facility_name, site_type, first_year = year) %>%
  gt()
```
...
nc_campground_site_counts_by_site_type <- nc_history %>%
  ...
  #distinct(name_first_word, facility_id_group, site_type, .keep_all = TRUE) %>% 
  ##distinct(facility_id_group, facility_id, site_type, .keep_all = TRUE) %>% 
  #select(name_first_word, facility_id, facility_name, facility_type_description, site_type, facility_id_group) %>%
  group_by(facility_id_group) %>%
  mutate(facility_name = as.list(facility_name),
...
#   count(year, facility_name, site_type, season, week_of_reservation,
#         name = "n_nights") %>%
...
top_occupancy_by_year_season <- nc_yday_occupancy %>%
  filter(between(year, year_start, year_stop)) %>%
  filter(site_type != "management") %>%
  group_by(year, facility_name, season, site_type, n_campsites_max) %>%
  summarize(avg_occupancy = mean(occupancy),
            median_occupancy = median(occupancy)) %>%
  ungroup() %>%
  arrange(year, season, -avg_occupancy) %>%
  group_by(year, season, site_type) %>%
  slice_max(n = 1, order_by = avg_occupancy) %>%
  ungroup() 
...
nc_week_occupancy %>%
  filter(year >= year_lower_bound,
         site_type != "management") %>%
  group_by(year, site_type, week_of_reservation) %>% #season, 
...
max_occupancy_by_year_season %>%
  count(facility_name, site_type, name = "n_days") %>%
  group_by(facility_name) %>%
...
#   count(facility_name, site_type) %>%
#   mutate(temp = 1) %>%
#   pivot_wider(names_from = site_type, values_from = temp)
#   arrange(-n) %>%
#   # since every park and site type will be at max occupancy at least once (by definition!), we'll ignore those
#   filter(n > 1) %>%
#   gt() %>%
...
```{r reservation-pattern-tent, fig.height=12, fig.width=12, out.width="100%"}
nc_history_for_plot %>%
...
product_id_counts <- tribble(
  ...
  "Additional cases in which multiple facilities use the sampe product_id values across more detailed, multiple, original site_type categories"
) %>%
...
* Look at growth only in facilities + site_type that existed in 2009. This would avoid adding to the growth rate due to facilities that converted camp sites to reservable during the years of interest (and thus started showing up in the data).

* DONE: As noted in [What's possible given these data sets?] the site-type categories {standard, tent, rv} overlap. I could recategorize standard sites to tent camping except in RV-only campsites, in which case I would allocate them to rv camping.
...
* In time series analysis, 
    ...
    - Would a hierarchical time series model be helpful? We could use facility_id_group as the higher-level series and keep using facility_name + site_type as the lower-level series.
