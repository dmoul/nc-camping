...
nc_campgrounds_year_min %>%
  filter(ignore) %>% 
  select(facility_id, facility_name, first_year = year) %>%
  gt()
```

<br><br>

The following campgrounds had at least one site_type reserveable in `r year_lower_bound` and added the site_type in the year noted below:
...
* Adjust nc_day, nc_week, nc_week_n_camps_occupied to use nc_history_detail instead (rather than relying on reservation start_date)
