...
```{r nc-map, out.width="100%", fig.height=6, fig.width=8}
ggplot() + 
...
max_occupancy_by_year_season %>%
  count(facility_name, site_type_simple, name = "n_sites") %>%
  arrange(-n_sites) %>%
  # since every park and site type will be at max occupancy at least once (by definition!), we'll ignore those
  filter(n_sites > 1) %>%
  gt() %>%
...
* DONE: filter on year_lower_bound in all appropriate places
* use facility_name_combined in all appropriate places
* DONE: fix graph zooming in knitr output
* DONE: fix plots that need larger fig.width and fig.height
