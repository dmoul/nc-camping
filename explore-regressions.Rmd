...
nc_camping_person_nights_and_revenue_site_type %>%
  ...
  ungroup() %>%
  mutate(site_type_simple = fct_reorder(site_type_simple, n_reservations),
         year = year + year_lower_bound - 1) %>% #view()
  ggplot(aes(x = year, y = n_reservations, fill = site_type_simple)) +
  geom_area() +
...
fit0.reservations2 <- 
  nc_camping_person_nights_and_revenue_site_type %>%
  mutate(temp = 1) %>%
  pivot_wider(names_from = site_type_simple, names_prefix = "dummy_", values_from = temp) %>%
...
As expected, adding the categories of `site_type_simple` as dummy variables reduces the standard error on the year.
...
## Add `site_type_simple`

```{r}
fit2.reservations <- lmer(data = df_facility_site_type,
            formula = n_reservations_c ~ 1 + site_type_simple + (1 | facility_first_word)
)
...
fit2.person_nights <- lmer(data = df_facility_site_type,
            formula = person_nights_c ~ 1 + site_type_simple + (1 | facility_first_word)
)
...
fit2.revenue <- lmer(data = df_facility_site_type,
            formula = revenue_in_year_c ~ 1 + site_type_simple + (1 | facility_first_word)
)
...
fit3.reservations <- lmer(data = df_facility_site_type,
      formula = n_reservations_c ~ 1 + site_type_simple + year + (1 | facility_first_word)
)
...
fit3.person_nights <- lmer(data = df_facility_site_type,
      formula = person_nights_c ~ 1 + site_type_simple + year + (1 | facility_first_word)
)
...
fit3.revenue <- lmer(data = df_facility_site_type,
      formula = revenue_in_year_c ~ 1 + site_type_simple + year + (1 | facility_first_word)
)
...
## Change `site_type_simple` to be a level of the model

```{r}
fit4.reservations <- lmer(data = df_facility_site_type,
      formula = n_reservations_c ~ 1 + year + (1 | site_type_simple) + (1 | facility_first_word)
)
...
fit4.person_nights <- lmer(data = df_facility_site_type,
      formula = person_nights_c ~ 1 + year + (1 | site_type_simple) + (1 | facility_first_word)
)
...
fit4.revenue <- lmer(data = df_facility_site_type,
      formula = revenue_in_year_c ~ 1 + year + (1 | site_type_simple) + (1 | facility_first_word)
)
...
```{r}
fit2b.reservations <- brm(n_reservations ~ 1 + site_type_simple + (1 | facility_name_original), 
...
```{r}
fit3b.reservations <- brm(n_reservations ~ 1 + site_type_simple + year + (1 | facility_name_original), 
                          data = df_facility_site_type,
                          save_pars = save_pars(all = TRUE),
                          cores  = 4,
                          silent = 2, # fewer messages while running
...
Turning site_type_simple into a group effect in fit4b...
...
fit4b.reservations <- brm(n_reservations ~ 1 + year + (1 | site_type_simple) + (1 | facility_id), 
                          data = df_facility_site_type,
                          cores  = 4,
                          silent = 2, # fewer messages while running
...
```{r}
fit4b.reservations <- brm(n_reservations ~ 1 + year + (1 | site_type_simple) + (1 | facility_name_original), 
                          data = df_facility_site_type,
                          save_pars = save_pars(all = TRUE),
                          cores  = 4,
                          silent = 2, # fewer messages while running
...
mcmc_areas(posterior,
  ...
  labs(title = "NC Camping Reservations - mulit-level model 'fit3b.reservations'", 
       subtitle = "formula = n_reservations ~ 1 + year + (1 | site_type_simple) + (1 | facility_name_original)")
```
...
ppc_intervals_grouped(
  ...
  yrep = posterior_predict(fit3b.reservations),
  x = df_facility_site_type$year,
  group = df_facility_site_type$site_type_simple,
  prob = 0.5
) +
  labs(
    x = "Year",
    y = "n_reservations",
    title = "fit3b.reservations: 50% posterior predictive intervals \nvs observed n_reservations",
    subtitle = "grouped by site_type_simple"
  ) +
  panel_bg(fill = "gray95", color = NA) +
  grid_lines(color = "white")
```
...
What if we group by both facility and site_type_simple? Nope: can't do both at once:
...
ppc_intervals_grouped(
  ...
  yrep = posterior_predict(fit4b.reservations),
  x = df_facility_site_type$year,
  group = c(df_facility_site_type$site_type_simple, df_facility_site_type$facility_first_word),
  prob = 0.5
) +
  labs(
    x = "Year",
...
Let's consider fit4b.reservations, where `site_type_simple` and `facility_name_original` are the groups in the hierarchical model.
...
nc_camping_person_nights_and_revenue_site_type %>%
  group_by(year, facility_name_original, site_type_simple) %>%
  summarize(n_reservations = sum(n_reservations)) %>%
  ungroup() %>%
...
nc_camping_person_nights_and_revenue_site_type %>%
  ...
  scale_y_continuous(labels = label_number_si()) +
  scale_fill_viridis_d(end = 0.92, direction = -1) +
  facet_grid(facility_name_original ~ site_type_simple, scales = "free_y") + #, ncol = 4, scales = "free_y") +
  theme(legend.position = "top") +
  labs(title = "NC camping n_reservations",
       subtitle = glue("Y scale varies. {year_lower_bound}-{year_end}"),
       x = NULL,
